...
Records are authored as `docs/adrs/NNNN-slug.yml` against the librarian's `okf-yaml`
schema, adopted strictly; every `.md` beside them is generated by `make adrs`
(ADR 0048). Edit the YAML. A record states its argument in named fields rather than in
prose sections: `problem.symptom` / `pain_point`, `decision.given` / `we_prefer` (which
must name the rejected alternative) / `because` / `unless` / `in_practice`, and
`consequences.pros` / `cons` — a record with no `cons` has not been thought about.
`make adrs-check` validates, checks OKF conformance, and fails on a stale file.
