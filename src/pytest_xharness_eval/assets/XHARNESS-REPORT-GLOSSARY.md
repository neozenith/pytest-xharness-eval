...
report.html?effort=high,xhigh                         SweepOverview, either effort rung (ADR 0049)
report.html?skill=discovery&harness=claude&model=claude-opus-5   several at once (OR within a facet, AND across facets)
...
Playwright sweep, one deterministic slug per state, including the two empty states a
real sweep produces (a harness × model pair and a harness × effort pair that match no
session). An absent `skill`, `harness`,
`model` or `effort` means **every** value of that facet, and so does a present-but-empty one
...
| `NavSidebar` | `NavToggle`, an Overview link, then skill → suite → harness → model (one branch per effort rung, `model · rung`, in ladder order; a cell that named no rung keeps the bare model) → one entry per session (verdict dot, case, short id); the open session lists section jump-links | the collapsible left navigation; every tree level starts collapsed (the reader opts in, and the active session's ancestors open themselves); collapse is remembered locally, never routed |
| `ReportHeader` | `ReportTitle`, `ReportMeta`, `ThemeToggle` | sticky chrome; `ReportTitle` is the report's name on the sweep and the `eval · session · harness · model [· effort]` tuple inside a `SessionView`, the rung (`ReportTitleEffort`) only when the cell was sent one; the browser tab keeps the page's fixed title |
...
| `TokenWaterfallAggregateChart` | the same six categories and total as `TokenWaterfallChart`, averaged over every run in view: one column per turn index plus `baseline` and `total`, with a min–max whisker per column | where the tokens went, across many runs rather than one — the decomposition to `TokenAccumulationChart`'s accumulation, over the same filtered population. Each column is the arithmetic mean over the runs that *reached* that turn (a short run is missing from a late turn, never zero in it), so the stack's top is the mean running total and the whisker brackets the real runs behind it; the hover reports how many runs each column averages. It pools every arm in the filtered set (harness × model × effort rung) into that one mean, unlike the accumulation chart beside it, which draws one line per arm; when more than one arm is in view its note says so, with the count, and that filtering to one arm reads a single experiment. The `total` column is each run's own final total, so on runs of unequal length it sits above the last turn's column rather than level with it |
...
The grouping key is **skill × case × harness × model × effort** — the very key
`accumulationGroups` draws with (one function, not two spellings of it), so one summary
row is one line of the `TokenAccumulationChart` above it and the two read as a pair. A
chart line is labelled by its suite file; when two lines would share that label (two
cases of one suite, or one suite name under two skills) both are qualified as
`skill/suite::case`. A run with no per-call ledger has a summary row but no line.
A capture from before ADR 0049 (no `effort` key) and a row whose `effort` is empty both
read as no rung: the page folds them to null when it loads `index.json`. Rows come out in that
key order ascending (a null skill last; effort by ladder position, `low` before `max`,
a group that named no rung after every rung). Effort is in the key because two rungs of
one model are two experiments (ADR 0049): averaged into one row they would print a mean
cost neither arm paid.
...
| Column | What it is |
|--------|------------|
| `skill`, `case`, `harness`, `model`, `effort` | the group's key; `effort` reads `–` for a group that named no rung |
